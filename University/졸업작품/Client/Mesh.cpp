...
CAssetMesh::CAssetMesh(ID3D11Device *pd3dDevice, const std::string& filename, unsigned int type) : CMeshTexturedIlluminated(pd3dDevice)
{
	...
	m_bcBoundingCube.m_d3dxvMinimum = D3DXVECTOR3(-12, -24, -12);
	m_bcBoundingCube.m_d3dxvMaximum = D3DXVECTOR3(+12, +24, +12);
...
CCharacterMesh::CCharacterMesh(ID3D11Device *pd3dDevice, float scale, const std::string& filename) : CMeshTexturedIlluminated(pd3dDevice)
{
	...
	for (int i = 0; i < m_nVertices; ++i)
	{
		m_pd3dxvPositions[i] *= (scale);
	}

	for (int i = 0; i < m_nIndices; i++)
	{
		in >> m_pnIndices[m_nIndices - i - 1];
	}
	in.close();//file close
	m_d3dPrimitiveTopology = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
...
CSkyDomeMesh::CSkyDomeMesh(ID3D11Device *pd3dDevice) : CMeshTextured(pd3dDevice)
{
	string filename[3] = { "SkyDome/SkyDome.data","SkyDome/Stars.data","SkyDome/Moon.data" };
	std::ifstream in[3];
	for (int i = 0; i < 3; i++)
	...
	int Vertices[3];//skydome, stars, moon
	int Indices[3];

	for (int i = 0; i < 3; i++)
	{
		in[i] >> ignore;//MESH_DATA
		in[i] >> ignore >> Vertices[i];//VertexCount	
		in[i] >> ignore >> Indices[i];//IndexCount;
		in[i] >> ignore >> ignore;//BoneCount;
		in[i] >> ignore >> ignore;//AnimationClips;

		m_nVertices += Vertices[i];
		m_nIndices += Indices[i];
	}
	...
	for (int i = 0; i < 3; i++)
	{
		for (int nowVtxNum = 0; nowVtxNum < Vertices[i]; ++nowVtxNum)
		{
			in[i] >> ignore >> m_pd3dxvPositions[offset].x >> m_pd3dxvPositions[offset].y >> m_pd3dxvPositions[offset].z;
			in[i] >> ignore >> ignore >> ignore >> ignore;//normal;
			in[i] >> ignore >> pd3dxvTexCoords[offset].x >> pd3dxvTexCoords[offset].y;
			offset++;
		}
		in[i] >> ignore;
	}
	if (m_nVertices != offset)
	{
		//cout << "m_nVertices : " << m_nVertices << endl;
		//cout << "offset :" << offset << endl;
		//cout << "SkyBox Vertices Set error: Please try reconfirm." << endl;
	}
	offset = 0;
	int indexOffset = 0;
	for (int i = 0; i < 3; i++)
	{
		for (int nowIdxNum = 0; nowIdxNum < Indices[i]; nowIdxNum++)
		{
			in[i] >> m_pnIndices[indexOffset];
			m_pnIndices[indexOffset] += offset;
			indexOffset++;
		}
		offset += Vertices[i];
		in[i].close();//file close
	}
	if (m_nIndices != indexOffset)
	{
		cout << "m_nIndex : " << m_nIndices << endl;
		cout << "Index offset :" << indexOffset << endl;
		cout << "SkyBox Indexs Set error: Please try reconfirm." << endl;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////
// minimap
CMinimapPlayerMesh::CMinimapPlayerMesh(ID3D11Device *pd3dDevice) : CMeshDiffused(pd3dDevice)
{
    m_nVertices = 6;
    ...
    // Default Topology : Triangle List

    m_pd3dxvPositions[0] = D3DXVECTOR3(0.0f, 0.05f, 0.0f);
    m_pd3dxvPositions[1] = D3DXVECTOR3(0.1f, 0.1f, 0.0f);
    m_pd3dxvPositions[2] = D3DXVECTOR3(0.0f, -0.1f, 0.0f);
    m_pd3dxvPositions[3] = D3DXVECTOR3(0.0f, 0.05f, 0.0f);
    m_pd3dxvPositions[4] = D3DXVECTOR3(0.0f, -0.1f, 0.0f);
    m_pd3dxvPositions[5] = D3DXVECTOR3(-0.1f, 0.1f, 0.0f);
    ...
    // color buffer
    D3DXCOLOR* pd3dxvColor = new D3DXCOLOR[m_nVertices];
    for (int i = 0; i < m_nVertices; ++i)
        pd3dxvColor[i] = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);

    d3dBufferDesc.ByteWidth = sizeof(D3DXCOLOR) * m_nVertices;

    d3dBufferData.pSysMem = pd3dxvColor;
    ...
    int nBuffers = 2;
    ID3D11Buffer *ppd3dBuffers[2] = { m_pd3dPositionBuffer, m_pd3dColorBuffer };
    UINT pnBufferStrides[2] = { sizeof(D3DXVECTOR3), sizeof(D3DXVECTOR4) };
    UINT pnBufferOffsets[2] = { 0, 0 };

    AssembleToVertexBuffer(nBuffers, ppd3dBuffers, pnBufferStrides, pnBufferOffsets);
}

CMinimapPlayerMesh::~CMinimapPlayerMesh()
{
}

CMinimapEnemyMesh::CMinimapEnemyMesh(ID3D11Device *pd3dDevice) : CMeshDiffused(pd3dDevice)
{
    m_nVertices = 9;
    ...
    // Default Topology : Triangle List
    float fRadius = 2.0f;
    m_pd3dxvPositions[0] = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
    m_pd3dxvPositions[1] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(0.0f)), 0.0f, fRadius * sinf(D3DXToRadian(0.0f)));
    m_pd3dxvPositions[2] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(45.0f)), 0.0f, fRadius * sinf(D3DXToRadian(45.0f)));
    m_pd3dxvPositions[3] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(90.0f)), 0.0f, fRadius * sinf(D3DXToRadian(90.0f)));
    m_pd3dxvPositions[4] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(135.0f)), 0.0f, fRadius * sinf(D3DXToRadian(135.0f)));
    m_pd3dxvPositions[5] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(180.0f)), 0.0f, fRadius * sinf(D3DXToRadian(180.0f)));
    m_pd3dxvPositions[6] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(225.0f)), 0.0f, fRadius * sinf(D3DXToRadian(225.0f)));
    m_pd3dxvPositions[7] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(270.0f)), 0.0f, fRadius * sinf(D3DXToRadian(270.0f)));
    m_pd3dxvPositions[8] = D3DXVECTOR3(fRadius * cosf(D3DXToRadian(315.0f)), 0.0f, fRadius * sinf(D3DXToRadian(315.0f)));
    ...
    D3DXCOLOR* pd3dxvColor = new D3DXCOLOR[m_nVertices];
    for (int i = 0; i < m_nVertices; ++i)
        pd3dxvColor[i] = D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f);
    ...
    // index buffer
    m_nIndices = 24;
    m_pnIndices = new UINT[24];

    for (int i = 0; i < 7; ++i)
    {
        m_pnIndices[3 * i] = 0;
        m_pnIndices[3 * i + 1] = i + 2;
        m_pnIndices[3 * i + 2] = i + 1;
    }
    m_pnIndices[21] = 0;
    m_pnIndices[22] = 1;
    m_pnIndices[23] = 8;
...
CMinimapEnemyMesh::~CMinimapEnemyMesh()
{
}

// radar stencil circle
CMinimapRadarMesh::CMinimapRadarMesh(ID3D11Device *pd3dDevice) : CMeshTextured(pd3dDevice)
{
    m_nVertices = 33;
    ...
    for (int i = 0; i < m_nVertices - 1; ++i)
        m_pd3dxvPositions[i] = D3DXVECTOR3(cosf(D3DXToRadian(11.25f * i)), sinf(D3DXToRadian(11.25f * i)), 0.0f);
    m_pd3dxvPositions[32] = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
    ...
    // tex buffer
    D3DXVECTOR2* pd3dxvTexs = new D3DXVECTOR2[m_nVertices];
    for (int i = 0; i < m_nVertices - 1; ++i)
        pd3dxvTexs[i] = D3DXVECTOR2(0.5f + 0.5f * cosf(D3DXToRadian(22.5f * i)), 0.5f + 0.5f * sinf(D3DXToRadian(22.5f * i)));
    pd3dxvTexs[32] = D3DXVECTOR2(0.5f, 0.5f);

    d3dBufferDesc.ByteWidth = sizeof(D3DXVECTOR2) * m_nVertices;

    d3dBufferData.pSysMem = pd3dxvTexs;

    pd3dDevice->CreateBuffer(&d3dBufferDesc, &d3dBufferData, &m_pd3dTexCoordBuffer);

    // index buffer
    m_nIndices = 96;
    m_pnIndices = new UINT[96];

    for (int i = 0; i < 31; ++i)
    {
        m_pnIndices[3 * i] = 32;
        m_pnIndices[3 * i + 1] = i + 1;
        m_pnIndices[3 * i + 2] = i;
    }
    m_pnIndices[93] = 32;
    m_pnIndices[94] = 0;
    ...
    // assemble vertex buffer
    int nBuffers = 2;
    ID3D11Buffer *ppd3dBuffers[2] = { m_pd3dPositionBuffer, m_pd3dTexCoordBuffer };
    UINT pnBufferStrides[2] = { sizeof(D3DXVECTOR3), sizeof(D3DXVECTOR2) };
    UINT pnBufferOffsets[2] = { 0, 0 };
...
CMinimapRadarMesh::~CMinimapRadarMesh()
{
}

// radar texture mapping plane
CMinimapRadarPlaneMesh::CMinimapRadarPlaneMesh(ID3D11Device *pd3dDevice) : CMeshTextured(pd3dDevice)
{
    m_nVertices = 6;
    ...
    m_pd3dxvPositions[0] = D3DXVECTOR3(-1.0f, 1.0f, 0.0f);
    m_pd3dxvPositions[1] = D3DXVECTOR3(1.0f, 1.0f, 0.0f);
    m_pd3dxvPositions[2] = D3DXVECTOR3(1.0f, -1.0f, 0.0f);
    m_pd3dxvPositions[3] = D3DXVECTOR3(-1.0f, 1.0f, 0.0f);
    m_pd3dxvPositions[4] = D3DXVECTOR3(1.0f, -1.0f, 0.0f);
    m_pd3dxvPositions[5] = D3DXVECTOR3(-1.0f, -1.0f, 0.0f);
    ...
    pd3dxvTexs[0] = D3DXVECTOR2(0.01f, 0.01f);
    pd3dxvTexs[1] = D3DXVECTOR2(0.99f, 0.01f);
    pd3dxvTexs[2] = D3DXVECTOR2(0.99f, 0.99f);
    pd3dxvTexs[3] = D3DXVECTOR2(0.01f, 0.01f);
    pd3dxvTexs[4] = D3DXVECTOR2(0.99f, 0.99f);
    pd3dxvTexs[5] = D3DXVECTOR2(0.01f, 0.99f);
...
CMinimapRadarPlaneMesh::~CMinimapRadarPlaneMesh()
{
}

// radar grid plane
CMinimapRadarGridPlaneMesh::CMinimapRadarGridPlaneMesh(ID3D11Device *pd3dDevice) : CMeshTextured(pd3dDevice)
{
    m_nVertices = 6;
...
CMinimapRadarGridPlaneMesh::~CMinimapRadarGridPlaneMesh()
{
}
...
CGSRectMesh::CGSRectMesh(ID3D11Device *pd3dDevice, float width, float height) : CMeshTextured(pd3dDevice){
	m_nVertices = 1;
	...
	pd3dxvPositions[0] = D3DXVECTOR3(0.0f, 0.0f, 0.0f);

	
	m_pd3dPositionBuffer = CreateBuffer(pd3dDevice, sizeof(D3DXVECTOR3), m_nVertices, pd3dxvPositions, D3D11_BIND_VERTEX_BUFFER, D3D11_USAGE_DYNAMIC, 0);
