...
void CGameObject::Move(D3DXVECTOR3 d3dxvPosition) {
	m_d3dxmtxLocal._41 += d3dxvPosition.x;
	//m_d3dxmtxLocal._42 += d3dxvPosition.y;
	m_d3dxmtxLocal._43 += d3dxvPosition.z;
}
...
void CGameObject::SetLookAndGenerate(D3DXVECTOR3 d3dxvLook_vector) {
	
	
	D3DXVECTOR3 d3dxvUp_vector = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
	D3DXVECTOR3 d3dxv_lookc_vector = d3dxvLook_vector;
	D3DXVECTOR3 d3dxvRight_vector;
	D3DXVec3Normalize(&d3dxv_lookc_vector, &d3dxv_lookc_vector);
	D3DXVec3Cross(&d3dxvRight_vector, &d3dxvUp_vector, &d3dxv_lookc_vector);
	D3DXVec3Normalize(&d3dxvRight_vector, &d3dxvRight_vector);

	m_d3dxmtxLocal._11 = d3dxvRight_vector.x;
	m_d3dxmtxLocal._12 = d3dxvRight_vector.y;
	m_d3dxmtxLocal._13 = d3dxvRight_vector.z;

	m_d3dxmtxLocal._21 = d3dxvUp_vector.x;
	m_d3dxmtxLocal._22 = d3dxvUp_vector.y;
	m_d3dxmtxLocal._23 = d3dxvUp_vector.z;

	m_d3dxmtxLocal._31 = d3dxv_lookc_vector.x;
	m_d3dxmtxLocal._32 = d3dxv_lookc_vector.y;
	m_d3dxmtxLocal._33 = d3dxv_lookc_vector.z;
...
void CGameObject::UpdateShaderVariable(ID3D11DeviceContext *pd3dDeviceContext, D3DXMATRIX *pd3dxmtxWorld)
{
    ...
    pcbWorldMatrix->m_d3dxmtxWorld = *pd3dxmtxWorld;
...
void CGameObject::CreateRasterizerState(ID3D11Device *pd3dDevice)
{
    ...
    pd3dDevice->CreateRasterizerState(&d3dRasterizerDesc, &m_pd3dRasterizerState_Wire);
}
...
void CGameObject::Render(ID3D11DeviceContext *pd3dDeviceContext, CCamera *pCamera)
{
	...
	if ((m_pParent != NULL) && (m_Objecttype == ObjectType::player_weapon))
	{
		D3DXMATRIX mtxLocalWorld = m_pParent->m_d3dxmtxLocal;
		D3DXMATRIX mtxRightHand;
		m_pParent->GetMesh()->GetHandMatrix(mtxRightHand);

		D3DXMatrixIdentity(&m_d3dxmtxWorld);

		m_d3dxmtxWorld = mtxRightHand*mtxLocalWorld;//2016.5.13 ¹ÚÁ¾Çõ!
	}
    ...
    if(m_bWireFrame)
	    pd3dDeviceContext->RSSetState(m_pd3dRasterizerState_Wire);
    else
...
void CSkyBox::Render(ID3D11DeviceContext *pd3dDeviceContext, CCamera *pCamera)
{
	...
	pd3dDeviceContext->DrawIndexed(732, 0, 0);

	pd3dDeviceContext->OMSetDepthStencilState(NULL, 1);
}

// radar
void CRadar::CreateDepthStencilState(ID3D11Device *pd3dDevice)
{
    D3D11_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
    ...
    d3dDepthStencilDesc.FrontFace.StencilFunc = D3D11_COMPARISON_GREATER;
    d3dDepthStencilDesc.BackFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
    d3dDepthStencilDesc.BackFace.StencilDepthFailOp = D3D11_STENCIL_OP_KEEP;
    d3dDepthStencilDesc.BackFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
    d3dDepthStencilDesc.BackFace.StencilFunc = D3D11_COMPARISON_NEVER;

    pd3dDevice->CreateDepthStencilState(&d3dDepthStencilDesc, &m_pd3d_DSS_Radar_Step_1);
    ...
    pd3dDevice->CreateDepthStencilState(&d3dDepthStencilDesc, &m_pd3d_DSS_Radar_Step_2);

    d3dDepthStencilDesc.DepthEnable = true;
    d3dDepthStencilDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
    d3dDepthStencilDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
    d3dDepthStencilDesc.StencilEnable = true;
    d3dDepthStencilDesc.StencilReadMask = D3D11_DEFAULT_STENCIL_READ_MASK;
    d3dDepthStencilDesc.StencilWriteMask = D3D11_DEFAULT_STENCIL_WRITE_MASK;
    d3dDepthStencilDesc.FrontFace.StencilFailOp = D3D11_STENCIL_OP_KEEP;
    ...
    pd3dDevice->CreateDepthStencilState(&d3dDepthStencilDesc, &m_pd3d_DSS_Radar_Step_3);
}

void CRadar::CreateBlendState(ID3D11Device *pd3dDevice)
{
    D3D11_BLEND_DESC d3dBlendDesc;
    ZeroMemory(&d3dBlendDesc, sizeof(D3D11_BLEND_DESC));
    d3dBlendDesc.AlphaToCoverageEnable = false;
    d3dBlendDesc.IndependentBlendEnable = false;
    ZeroMemory(&d3dBlendDesc.RenderTarget[0], sizeof(D3D11_RENDER_TARGET_BLEND_DESC));
    ...
    HRESULT hr = pd3dDevice->CreateBlendState(&d3dBlendDesc, &m_pd3dBlendState);
}