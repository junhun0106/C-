...
{
    CNode *start = mHead.m_pNext;
    CNode *temp = nullptr;

    while (start->m_pNext != nullptr)
    {
        temp = start;
        start = start->m_pNext;
        delete temp;
...
void CBulletList::Add(D3DXVECTOR3 d3dxvPos, D3DXVECTOR3 d3dxvDir)
{
    if (m_nBulletCount >= MAX_BULLET) {}
    else
    {
        CNode* newNode = new CNode(d3dxvPos, d3dxvDir, mHead.m_pNext, m_nBulletCount++);
        mHead.m_pNext = newNode;
    }
}

void CBulletList::Remove(CNode *pDest)
{
    CNode* start = mHead.m_pNext;
    CNode* pred = &mHead;
...
{
    CNode *start = mHead.m_pNext;
    if (start == &mTail) return;
    else
    {
        while (start != &mTail)
        {
            start->m_fLifeTime -= deltaTime;

            if (start->m_fLifeTime <= 0.0f)
            {
                Remove(start);
                return;
            }
            else
                start->m_d3dxvPos += start->m_d3dxvDir * start->BULLET_SPEED * deltaTime;
...
void CPlayer::Rotate(float x, float y, float z)
{
    ...
    if ((nCurrentCameraMode == FIRST_PERSON_CAMERA) || (nCurrentCameraMode == THIRD_PERSON_CAMERA))
    {
        ...
        {
            m_fPitch += x * 4.0f;
            if (m_fPitch > +360.0f) { x -= (m_fPitch - 360.0f); m_fPitch = +360.0f; }
            if (m_fPitch < -360.0f) { x -= (m_fPitch + 360.0f); m_fPitch = -360.0f; }
        }
        if (y != 0.0f)
        {
...
void CPlayer::Update(float fTimeElapsed)
{
    ...
    if (nCurrentCameraMode == THIRD_PERSON_CAMERA) m_pCamera->SetLookAt(m_d3dxvPosition + D3DXVECTOR3(0.0f, -m_fPitch * 0.01f + 1.5f, 0.0f));
...
void CTerrainPlayer::ChangeCamera(ID3D11Device *pd3dDevice, DWORD nNewCameraMode, float fTimeElapsed)
{
    ...
    case THIRD_PERSON_CAMERA:
		...
		//m_pCamera->SetOffset(D3DXVECTOR3(0.0f, 4.0f, -5.0f));
		m_pCamera->SetOffset(D3DXVECTOR3(0.0f, 3.5f, -5.25f));
        m_pCamera->GenerateProjectionMatrix(1.01f, 5000.0f, ASPECT_RATIO, 60.0f);
        break;
    default:
        break;
...
void CTerrainPlayer::OnPlayerUpdated(float fTimeElapsed)
{
    ...
    float fHeight = pTerrain->GetHeight(d3dxvPlayerPosition.x, d3dxvPlayerPosition.z, bReverseQuad) + 1.3f;
...
void CTerrainPlayer::OnCameraUpdated(float fTimeElapsed)
{
    ...
    bool bReverseQuad = ((z % 2) != 0);
    float fHeight = pTerrain->GetHeight(d3dxvCameraPosition.x, d3dxvCameraPosition.z, bReverseQuad) + 1.3f;
